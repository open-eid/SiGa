...
class HashcodeContainerControllerUpdateHashcodeContainerRemoteSigningResponseExceptionHandlerTest extends ExceptionHandlerTestBase {
    ...
    protected ResultActions performRequestAndReturnResponse(Throwable toThrowInController) throws Exception {
        ...
        }).when(containerService).createContainer(anyList());

        JSONObject request = createCreateContainerRequest();
        return performPost("/hashcodecontainers", request);
    }
}
