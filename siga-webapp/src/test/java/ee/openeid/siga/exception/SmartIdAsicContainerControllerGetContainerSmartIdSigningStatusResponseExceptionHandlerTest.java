...
class SmartIdAsicContainerControllerGetContainerSmartIdSigningStatusResponseExceptionHandlerTest extends ExceptionHandlerTestBase {
    ...
    protected ResultActions performRequestAndReturnResponse(Throwable toThrowInController) throws Exception {
        ...
        String signatureId = "dummy-certificate-Id";

        doAnswer(invocation -> {
            throw toThrowInController;
        }).when(validator).validateContainerId(anyString());

        String url = "/containers/" + containerId + "/smartidsigning/" + signatureId +"/status";
