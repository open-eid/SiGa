...
class AsicContainerControllerUploadContainerExceptionHandlerTest extends ExceptionHandlerTestBase {
    ...
    protected ResultActions performRequestAndReturnResponse(Throwable toThrowInController) throws Exception {
        ...
        }).when(containerService).uploadContainer(anyString(), any());

        JSONObject request = createUploadContainerRequest("VGVzdC4=", "datafile.asice");
        return performPost("/upload/containers", request);
    }
}
