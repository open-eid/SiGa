...
class AsicContainerControllerCreateContainerExceptionHandlerTest extends ExceptionHandlerTestBase {
    ...
    protected ResultActions performRequestAndReturnResponse(Throwable toThrowInController) throws Exception {
        ...
        }).when(containerService).createContainer(anyString(), any());

        JSONObject request = createCreateContainerRequest();
        return performPost("/containers", request);
    }
}
