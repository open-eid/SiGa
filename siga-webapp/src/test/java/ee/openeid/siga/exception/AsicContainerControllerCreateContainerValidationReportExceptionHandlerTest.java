...
class AsicContainerControllerCreateContainerValidationReportExceptionHandlerTest extends ExceptionHandlerTestBase {
    ...
    protected ResultActions performRequestAndReturnResponse(Throwable toThrowInController) throws Exception {
        ...
        }).when(validationService).validateContainer(anyString(), any());

        JSONObject request = createUploadContainerRequest("VGVzdC4=", "datafile.asice");
        return performPost("/containers/validationreport", request);
    }
}
